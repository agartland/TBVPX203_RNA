...
"""sampleid P203_903_3006_0 is swapped with P203_903_3005_0 Still need to confirm.
The correction for now will be to reassign those sampleids in the cts and correct treatment
and sex will follow"""
...
cts_df = cts_df.rename({'P203_903_3006_0':'P203_903_3005_0',
                         'P203_903_3005_0':'P203_903_3006_0'}, axis=1)
...
tmp = cts_df.loc[cts_df['Unnamed: 0'].isin(ygenes)].set_index('Unnamed: 0').T
tmp = tmp.assign(sex=tmp.apply(lambda r: 'male' if r[ygenes].sum() > 0 else 'female' , axis=1))
sex = tmp.reset_index().rename({'index':'sampleid'}, axis=1)
...
samples = pd.merge(samples, sex[['sex', 'sampleid'] + ygenes], how='left', on='sampleid')
